				...
				$query = mysqli_query($conn, "SELECT *FROM product WHERE category='feature' ORDER BY product_id DESC") or die (mysqli_error());
					...
					while($fetch = mysqli_fetch_array($query))
						...
						$pid = $fetch['product_id'];
							...
							echo "<br />";
							echo '<a href="details.php?id='.$fetch['product_id'].'" style="padding:10px 10px 10px 10px; position:relative; letter-spacing: 1px; font-size: 20px; color:#fff; background-color: #b68922; border-radius: 25px; border: 2px solid #fff558;  font-weight:bold;" class="button"><span id="stylegamaysad">Order </span></a>';
